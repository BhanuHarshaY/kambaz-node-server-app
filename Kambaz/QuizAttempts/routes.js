...
export default function QuizAttemptRoutes(app) {
  ...
  // Helper function to grade answers (supports multiple correct answers with partial credit)
    ...
    const gradedAnswers = plainAnswers.map(answer => {
      ...
      switch (question.type) {
        case "MULTIPLE_CHOICE":
          ...
          // Get selected choices (supports both single and multiple selection)
          const selectedIds = answer.selectedChoiceIds ||
            (answer.selectedChoiceId ? [answer.selectedChoiceId] : []);

          if (correctChoices.length === 0) {
            // No correct answers marked - give 0 points
            isCorrect = false;
            pointsEarned = 0;
          } else if (correctChoices.length === 1) {
            // Single correct answer - all or nothing
            isCorrect = selectedIds.length === 1 &&
              correctChoices[0]._id === selectedIds[0];
            pointsEarned = isCorrect ? question.points : 0;
          } else {
            // Multiple correct answers - partial credit
            const pointsPerCorrect = question.points / correctChoices.length;

            // Count correct selections
            const correctSelections = selectedIds.filter(id =>
              correctChoices.some(c => c._id === id)
            ).length;

            // Count incorrect selections (penalty)
            const incorrectSelections = selectedIds.filter(id =>
              incorrectChoices.some(c => c._id === id)
            ).length;

            // Calculate points: +1 for each correct, -1 for each wrong
            pointsEarned = Math.max(0,
              (correctSelections - incorrectSelections) * pointsPerCorrect
            );

            // Round to 2 decimal places
            pointsEarned = Math.round(pointsEarned * 100) / 100;

            // Consider fully correct only if ALL correct answers selected and NONE wrong
            isCorrect = correctSelections === correctChoices.length && incorrectSelections === 0;
          }
          break;
